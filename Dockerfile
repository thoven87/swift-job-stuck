...
RUN cp "$(swift build --package-path /build -c release --show-bin-path)/SwiftJobStuck" ./
...
COPY --from=build --chown=hummingbird:hummingbird /staging/SwiftJobStuck /app/SwiftJobStuck
